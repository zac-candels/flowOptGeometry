...
def CL_spread(datadir):
    ...
    return [np.asarray(right_CL_disp_vec), np.asarray(left_CL_disp_vec), tinc, tend]


# Directory where data is stored

grad_type = ["continuous", "sawtooth", "two-six", "noSizeGrad"]

timeConvFac = 75187.9699 # 1 second is equivalent to 75187.96 lattice units
spaceConvFac = 50 # 1mm is equivalent to 50 lattice units

fig, ax = plt.subplots()
for idx in range(len(grad_type)):

    datadir = "./" + grad_type[idx] + "/data/"
    right_CL_disp_vec, left_CL_disp_vec, tinc, tend = CL_spread(datadir)      


    t = range(0, tend, tinc)
    t = np.asarray(t) / timeConvFac
    left_CL_disp_vec = left_CL_disp_vec / spaceConvFac
    ax.plot(t, left_CL_disp_vec, linewidth=2, label=grad_type[idx])

ax.set_ylabel(r"Distance [mm]", fontsize = 15)
