...
int initBoundary(const int k) {
    ...
    int patterned_lx = lx - reservoirFrontWall_x + dx;
    int nbPost = 10;
    int nbPostZ = 10;
    double postfraction = 0.3;
    double postfractionz = 0.4;
    int postheight = reservoirOutletHeight;

    // Pillars in x and z direction with id 1, we will later apply bounce back to these nodes
    if ( xx >= patterned_lx && xx % (patterned_lx / nbPost) < postfraction * patterned_lx / nbPost && // Take the remainder after dividing the current x coordinate by the lx/nbPost.
...
auto initBinary() {
    ...
    WellBalancedCH<0,666,Lattice, DefaultTraitWellBalancedCH2<0,666,Lattice>::template AddProcessor<std::tuple<GradientsMultiStencil<ChemicalPotential<0>,CentralQBounceBack,CentralXYZBounceBack>>>> binary;
