...
class Gradients : public AddOnBase {
   public:
    ...
    inline void setWettingPrefactor(double v) { mGradientStencil.setPrefactor(v); }
    inline void setWettingPrefactor(const std::vector<double>& v) { mGradientStencil.setPrefactor(v); }
    inline void setWettingPrefactor(const std::vector<int>& ids, const std::vector<double>& prefs) {
        mGradientStencil.setPrefactor(ids, prefs);
}
    ...
    // NEW: Setter for boundary-specific prefactor
    inline void setWettingPrefactorForBoundary(int id, std::vector<double> value) {
        mGradientStencil.setPrefactorForBoundary(id, value);
    }
...
class GradientsMulti : public AddOnBase {
   public:
    ...
    inline void setWettingPrefactor(double v) {
        std::apply([&](auto&... grad) { (grad.setWettingPrefactor(v), ...); }, mt_ParamGradients);
    }
    inline void setWettingPrefactor(const std::vector<double>& v) {
        std::apply([&](auto&... grad) { (grad.setWettingPrefactor(v), ...); }, mt_ParamGradients);
    }
    inline void setWettingPrefactor(const std::vector<int>& ids, const std::vector<double>& prefs) {
        std::apply([&](auto&... grad) { (grad.setWettingPrefactor(ids, prefs), ...); }, mt_ParamGradients);
    }
    inline void setWettingPrefactor(int boundaryID, double value) {
        mt_ParamGradients.setPrefactor(boundaryID, value);
    }    
    inline void setWettingPrefactorForBoundaries(const std::vector<int>& ids,
                                             const std::vector<double>& values) {
    if (ids.size() != values.size()) {
        std::cerr << "Error: IDs and values must have the same size!" << std::endl;
        return;
    }

    // Loop over each ID/value pair
    for (size_t i = 0; i < ids.size(); ++i) {
        int boundaryID = ids[i];
        double value = values[i];

        // Apply to all gradient instances
        std::apply([boundaryID, value](auto&... gradient) {
            (..., gradient.setWettingPrefactorForBoundary(boundaryID, value));
        }, mt_ParamGradients);
    }
}
   ...
   inline void setInterfaceVal(double vaclewlue) {
    std::apply([vaclewlue](auto&... gradient) { (gradient.setInterfaceVal(vaclewlue), ...); }, mt_ParamGradients);
}
    ...
    inline std::vector<int> getBoundaryIDs() const {
    std::vector<int> all_ids;
    std::apply([&all_ids](auto&... gradient) {
        (..., ( 
            [&]{
                const auto& ids = gradient.getBoundaryIDs();
                if (!ids.empty())
    ...
    inline void setWettingPrefactorForBoundary(int id, std::vector<double> value) {
    std::apply([&](auto&... gradient) { (gradient.setWettingPrefactorForBoundary(id, value), ...); }, mt_ParamGradients);
    }
    // apply the wettingfractor using the boundary IDs
    // apply the wettingprefactor using the boundary IDs
    inline void applyWettingPrefactorByBoundaryIDs(const std::vector<int>& ids, const std::vector<double>& values) {
    // ... (optional: print info) ...
    std::apply([&ids, &values](auto&... gradient) {
        (..., ([&]{
            const auto& gradientIDs = gradient.getBoundaryIDs();
            if (gradientIDs.empty()) {
                // ... print skip message ...
                return;
            }
            // ... print boundary IDs ...
            for (const auto& gradientID : gradientIDs) {
                auto it = std::find(ids.begin(), ids.end(), gradientID);
                if (it != ids.end()) {
                    size_t idx = std::distance(ids.begin(), it);
                    if (idx < values.size()) {
                        gradient.setWettingPrefactor(gradientID, values[idx]);
                        // ... print set message ...
                    }
                }
            }
