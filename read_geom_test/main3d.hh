...
{
    int bdyID;
    bdyArray.resize(lx*ly*lz);

  // Read from the text file
    if (mpi.rank == 0)
    {
        const std::string path = "../../walls_Mixed_2.vtk";
        ...
        {
            std::cerr << "ERROR: Not a regular file: " << path << std::endl;
            return;
        }

        std::ifstream bdyFile(path);
        if (!bdyFile.is_open()) 
        {
            std::cerr << "ERROR: File exists but cannot be opened: " << path << std::endl;
            return;
        }
    ...
    std::cout << "bdyArray has size " << bdyArray.size() << std::endl;
    std::cout << "system has size " << lx * ly * lz << std::endl; 
