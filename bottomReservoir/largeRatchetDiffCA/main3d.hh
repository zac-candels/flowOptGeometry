...
void initParams(std::string inputfile) {
    ...
    */
    ...
    lz = std::max(a2, b2) + row_to_row_dist;
...
int initBoundary(const int k) {
    ...
    bool reservoirBackWallCond = ( xx >= reservoirBackWall_x ) && ( xx <= reservoirBackWall_x + dx ) && ( yy <= reservoirHeight );
    ...
    bool reservoirFrontWallCond = ( xx >= reservoirFrontWall_x - dx ) && ( xx <= reservoirFrontWall_x ) && ( yy > reservoirOutletHeight ) &&( yy <= reservoirHeight );
...
double initFluid(const int k) {
    ...
    double dist = std::max( abs(xx - posx), std::max( abs(yy - posy), abs(zz - posz) ) );
    ...
    if( ( yy > reservoirHeight + apertureHeight ) ) //&& ( zz >= z_c - apWidth2 ) && ( zz <= z_c + apWidth2 ) )
    {
        ...
        return (0.5 - 0.5 * tanh(2 * (dist - radius) / (4.0)));
    }
