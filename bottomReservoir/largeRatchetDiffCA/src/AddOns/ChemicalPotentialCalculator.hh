...
class ChemicalPotentialCalculatorNCompBoyerFinal : public AddOnBase {
 ...
 inline void ChemicalPotentialCalculatorNCompBoyerFinal<TN>::compute(const int k) {
    ...
    CVec.push_back(1-sum);
     ...
     for (int i=0;i<N-1;i++){
        ...
        for (int j=0;j<N;j++){
         ...
         getInstance<ChemicalPotential, N, Lattice>(i)[k] = chempot + 2 * mOmega * (ci < 0) * ci-
                                            2 * mOmega * ((1-ci) < 0) * (1-ci) + lambdaterm+0.8*P;//- 1./6.*chemforcecorr;// + lambdaterm+0.8*P;//*calcdH1(ci,ci*product)+product*(lambdadH2)+0.5*P;
    ...
    for (int j=0;j<N;j++){
     ...
     getInstance<ChemicalPotential, N, Lattice>(i)[k] = chempot + 2 * mOmega * (ci < 0) * ci-
                                            2 * mOmega * ((1-ci) < 0) * (1-ci)  + lambdaterm+0.8*P;// - 1./6.*chemforcecorr;// + lambdaterm+0.8*P;//mv_Lambda[i]*calcdH1(ci,ci*product)+product*(lambdadH2)+0.5*P;
