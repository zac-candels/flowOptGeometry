...
int initBoundary(const int k) {
    ...
    for(int i = 0; i <  Num_rows_of_ratchets; i++)
    {
        ...
        for (int j = 0; j < Num_ratchets_per_row; j++)
        {
            if( j < Num_ratchets_per_row /2 )
            {
                center_coords_arr[i][j][0] = x_pos_first_row_front + (j)*(tip_to_tip_dist);
            }
           else
           {
               center_coords_arr[i][j][0] = x_pos_first_row_behind - (Num_ratchets_per_row - j)*(tip_to_tip_dist);
           }
            center_coords_arr[i][j][1] = z_pos_first_row + i*row_to_row_dist;
