...
void initParams(std::string inputfile) {
    ...
    */
    ...
    offset = 15;
    channel_length = ( Num_ratchets_per_row + 1 )*( std::max(a2,c2) + inter_ratchet_dist );
    ...
    lz = std::max(a2, b2) + 2.5*row_to_row_dist;
    //lz = ((lz + 99) / 100) * 100;
...
int initBoundary(const int k) {
    ...
    for(int i = 0; i <  Num_rows_of_ratchets; i++)
    {
        ...
        for (int j = 0; j < Num_ratchets_per_row; j++)
        {
            ...
            if( j < Num_ratchets_per_row )
            {

                int anchor_pt = int( x_pos_first_row_front + (j)*(tip_to_tip_dist) ) % lx;
                center_coords_arr[i][j][0] = anchor_pt - a1p;
            }
          // else
          // {
          //     int anchor_pt = x_pos_first_row_behind - (Num_ratchets_per_row - j)*(tip_to_tip_dist);
          //     center_coords_arr[i][j][0] = anchor_pt - a1p;
          //}
