...
inline void MassLossCalculatorInterpolated::compute(int k) {
    ...
    if (Geometry<Lattice>::isBulkSolid(k) || mCalculate == false || k < Lattice::HaloSize ||
    ...
    const std::vector<double>& gradLiquid = getInstance<GradientOrderParameter, N, Lattice, NDIM>(mLiquidId);
    const std::vector<double>& gradGas = getInstance<GradientOrderParameter, N, Lattice, NDIM>(mLiquidId);
...
inline auto MassLossCalculatorInterpolated::findClosestKToInterface(const double& interfacedistance,
    ...
    for (int xi = 0; xi < 4 * round(mAlpha / 4.0); xi++) {
        for (int yi = 0; yi < 2 * (1 + (TTraits::Lattice::NDIM >= 2)) * round(mAlpha / 4.0); yi++) {
